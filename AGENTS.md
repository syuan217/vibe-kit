...
为多仓库(polyrepo)微服务团队提供 spec-driven AI 开发工作流:以 AGENTS.md 为跨 AI 工具的统一文档入口,以 vibe-clarify/build/verify 三阶段为需求开发流程(澄清/评审引擎来自 mattpocock/skills 外部依赖),以服务注册表协调跨应用需求。完整方案见 WORKFLOW.md(先读这个),目录与安装见 README.md。

- **kit 部分**(工具,随版本演进):`plugin/`(Claude/zcode/Kimi Code 插件,含 10 个 skills 与 `plugin/templates/` 应用仓库脚手架)、`prompts/`(与 skills 同源的 prompt 副本)、`scripts/`(初始化/校验/发版脚本)、`VERSION`
...
- `plugin/skills/<name>/SKILL.md` — 10 个 skills:vibe-clarify、vibe-build、vibe-verify(需求三阶段)、cross-app-spec、finalize-feature、rebuild-wiki、registry-sync、sync-docs、vibe-init、vibe-init-docs;每个 skill 就是一个 SKILL.md,**不带任何随附文件**;YAML frontmatter 必填 `name`/`description`,且 `name` 须等于目录名(CI 校验)
...
1. **同源以 SKILL.md 为唯一源,副本靠生成不靠手改**(范围按工作流的操作对象精确划定,不要机械补齐副本):
   - **三处同源**(skill + `prompts/` + `plugin/templates/app/prompts/`):finalize-feature、rebuild-wiki、sync-docs、vibe-clarify、vibe-build、vibe-verify——在应用仓库日常执行,三类用户都需要。
   ...
   - **外部依赖不入库**:流程实际调用的三个引擎是 grill-with-docs(及同源的 grilling/grill-me)、code-review、tdd,来自 mattpocock/skills,各应用仓库 `npx skills add` 整包安装,不进 vibe-kit 的 skill 目录、不出 prompt 副本。
