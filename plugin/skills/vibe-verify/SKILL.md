---
name: vibe-verify
description: 需求工作流阶段 3 — 调 code-review 对实现做两轴并行评审(Spec 轴:是否忠实 blueprint;Standards 轴:是否符合编码规范),偏差记进 spec.md,通过后提醒在 PR 评审通过、合并前跑 finalize-feature。Use when the user says "核对实现", "检查需求", "vibe-verify", "评审一下这次的改动", or right after /vibe-build completes.
---
