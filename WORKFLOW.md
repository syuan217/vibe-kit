...
hub(registry + 跨应用 specs + 公共 docs)与 kit(模板/脚本/插件)职责不同,可分可合,支持三种形态:

| 形态 | 做法 | 适用 |
|---|---|---|
| 合一(默认) | hub 就是 vibe-kit 仓库本身 | 小团队起步,最简单 |
| 分离 | `scripts/init-hub.sh <目录> --git` 生成独立 hub 仓库,团队共享;kit 仅作为工具与插件分发 | kit 公开而协调数据需私有;或多团队共用一份 kit |
| 本地个人 hub | init-hub 到本地目录,不建远端 | **仅限个人试用阶段** |

应用仓库通过根目录 `.vibe-hub` 文件定位 hub(vibe-init 时写入绝对路径,已自动加入 .gitignore——它是个人本地配置,每人各自生成);AI 与脚本的定位优先级:`.vibe-hub` → `$VIBE_HUB` 环境变量 → 询问。

> ⚠️ **团队协作时 hub 必须是共享 git 仓库**:总 spec 评审、registry 变更都依赖 PR;本地-only hub 会让协调数据在成员间分叉,恰好复活痛点三("文档在本地")。个人 hub 转正时:`cd <hub> && git init && git push`,然后各成员更新自己的 `.vibe-hub` 指向各自 clone 即可,目录结构不变。
